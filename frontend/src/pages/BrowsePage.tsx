...
export function BrowsePage() {
  ...
  const activeFilterCount = [category, city, minPrice || maxPrice].filter(Boolean).length;
  ...
  return (
    <div className="wrap mall page-top">
      ...
      <div className={`mall-section browse-toolbar${filtersOpen ? " is-open" : ""}`}>
        <div className="browse-filter-bar">
          <button
            type="button"
            className={filtersOpen ? "filter-toggle is-open" : "filter-toggle"}
            aria-expanded={filtersOpen}
            aria-controls="browse-filters"
            onClick={() => setFiltersOpen((open) => !open)}
          >
            <svg className="filter-toggle-icon" width="18" height="18" viewBox="0 0 24 24" fill="none" aria-hidden="true">
              <path
          ...
          {hasFilters ? (
            <button className="filter-clear-inline" type="button" onClick={clearFilters}>
              Clear all
            </button>
          ) : null}
        </div>
        <div id="browse-filters" className="filters">
